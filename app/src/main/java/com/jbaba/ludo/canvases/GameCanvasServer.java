...
public class GameCanvasServer extends AbsGameCanvas
{
    private Board board;
    ...
    public GameCanvasServer(Context context, Point point, String playerHouse)
    {
        super(context, point);
    ...
    private void initComputer()
    {
        ...
        playerClient.setPlayerName("Opponent");
    }
                ...
                while (isIncomingThreadRunning())
                {
                    ...
                    try
                    {
                        ...
                        else if(todo.charAt(0) == 'R')
                        {
                            ...
                            Log.d("GameCanvasServer", "Rolled value is " + a + " " + b);
                            setRolledValue(a, b);
    ...
    private void switchPlayer()
    {
        ...
        if(currPlayer.equals(playerServer))
        {
        ...
        else if(currPlayer.equals(playerClient))
        {
            if(!currPlayer.playerWon())
            {
                currPlayer = playerServer;
    ...
    public void declareGameOver()
    {
        ...
        intent.putExtra(DeclareWinnerActivity.HOSTORCLIENT, "Host");
    ...
    public boolean onTouchEvent(MotionEvent motionEvent)
    {
        switch(motionEvent.getAction() & MotionEvent.ACTION_MASK)
        {
            ...
            {
                if(currPlayer.equals(playerServer) && isShowing())
                {
                    ...
                    {
                        clickDice((int)motionEvent.getX(), (int)motionEvent.getY());
                        if(playerServer.makeCount((int)motionEvent.getX(), (int)motionEvent.getY(), getDieValueToCount(), true))
                        {
                            try {
                                Thread.sleep(100);
                            } catch (Exception ex) {
